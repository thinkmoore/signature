...
(define-splicing-syntax-class sig/defaults
  ...
  (pattern (~seq arg:arg/defaults ... result:result)
           #:attr arity (arity (length (filter identity (attribute arg.mandatory-id)))
                               ...
                               (map syntax->datum (filter identity (attribute arg.optional-kw)))
                               1)
           #:attr sig #`(#,@(apply append (map syntax->list (attribute arg.sig))) result)
           #:attr decl #`(#,@(apply append (map syntax->list (attribute arg.decl))))
  ...
  (pattern (~seq arg:arg/defaults ... (result:result ...))
           #:attr arity (arity (length (filter identity (attribute arg.mandatory-id)))
                               ...
                               (map syntax->datum (filter identity (attribute arg.optional-kw)))
                               (length (attribute result)))
           #:attr sig #`(#,@(apply append (map syntax->list (attribute arg.sig))) #,(apply append (map syntax->list (attribute result))))
           #:attr decl #`(#,@(apply append (map syntax->list (attribute arg.decl))))
           #:fail-when (duplicate-ids? (append (attribute arg.mandatory-id)
                                               (attribute arg.optional-id)
                                               (attribute arg.mandatory-kw-id)
  ...
  (pattern (~seq arg:arg/defaults ... .. rest:rest-arg result:result)
           #:attr arity (arity (length (filter identity (attribute arg.mandatory-id)))
                               ...
                               (map syntax->datum (filter identity (attribute arg.optional-kw)))
                               1)
           #:attr sig #`(#,@(apply append (map syntax->list (attribute arg.sig))) .. rest result)
           #:attr decl #`(#,@(apply append (map syntax->list (attribute arg.decl))) . rest.id)
           #:fail-when (duplicate-ids? (append (attribute arg.mandatory-id)
                                               (attribute arg.optional-id)
                                               (attribute arg.mandatory-kw-id)
                                               (attribute arg.optional-kw-id)
  ...
  (pattern (~seq arg:arg/defaults ... .. rest:rest-arg (result:result ...))
           #:attr arity (arity (length (filter identity (attribute arg.mandatory-id)))
                               ...
                               (map syntax->datum (filter identity (attribute arg.optional-kw)))
                               (length (attribute result)))
           #:attr sig #`(#,@(apply append (map syntax->list (attribute arg.sig))) .. rest #,(apply append (map syntax->list (attribute result))))
           #:attr decl #`(#,@(apply append (map syntax->list (attribute arg.decl))) . rest.id)
           #:fail-when (duplicate-ids? (append (attribute arg.mandatory-id)
                                               (attribute arg.optional-id)
                                               (attribute arg.mandatory-kw-id)
